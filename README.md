...
```bash
curl -fsSL https://raw.githubusercontent.com/theaaravagarwal/nexus/main/install.sh | NEXUS_INSTALL_DIR=/usr/local/bin NEXUS_VERSION=v0.2.1 sh
```
...
Windows hosts work too: install [btop4win](https://github.com/aristocratos/btop4win) (`winget install aristocratos.btop4win`) and make sure `btop` is on the SSH user's PATH. nexus detects a cmd.exe shell, runs btop4win inside a headless console (`conhost.exe --headless`) over a plain non-PTY session, and never involves WSL, so a cmd AutoRun hook that starts WSL for SSH logins is not triggered. The remote btop4win is torn down as soon as the Monitor session ends, and it runs from a temporary copy of its directory with braille graphs and rounded corners forced (btop4win's default config uses "tty" block graphs), so your own btop4win config is never modified. Compact telemetry and Fleet samples for Windows hosts use PowerShell performance counters (CPU, memory, network, uptime, NVIDIA GPUs via `nvidia-smi`); the load column shows the processor queue length.
...
- `internal/app`: the application, grouped by responsibility into CLI,
  connection, configuration, diagnostics, dashboard, theme, state, and transfer
  source files with colocated tests.
- `.github/workflows`: the minimal CI and release automation.
