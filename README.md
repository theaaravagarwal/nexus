...
Windows hosts work too: install [btop4win](https://github.com/aristocratos/btop4win) (`winget install aristocratos.btop4win`) and make sure `btop` is on the SSH user's PATH. nexus detects a cmd.exe shell, runs btop4win inside a headless console (`conhost.exe --headless`) over a plain non-PTY session, and never involves WSL, so a cmd AutoRun hook that starts WSL for SSH logins is not triggered. The remote btop4win is torn down as soon as the Monitor session ends. Compact telemetry and Fleet samples for Windows hosts use PowerShell performance counters (CPU, memory, network, uptime, NVIDIA GPUs via `nvidia-smi`); the load column shows the processor queue length.
